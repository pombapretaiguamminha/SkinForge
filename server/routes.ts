...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/convert", upload.single('objFile'), async (req, res) => {
    try {
      if (!req.file) {
      ...
      const objContent = req.file.buffer.toString('utf-8');
      
      // Parse OBJ file and convert to Blockbench format
      const convertedData = await convertObjToBlockbench(objContent, req.file.originalname);
...
async function convertObjToBlockbench(objContent: string, fileName: string) {
  ...
  // Scale vertices to reasonable Minecraft dimensions (typical range: -16 to 16)
...
function createCubesFromVertices(vertices: number[][], faces: number[][]): any[] {
  ...
  // Simple approach: create one main cube that encompasses the model
